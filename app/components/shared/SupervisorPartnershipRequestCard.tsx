...
export default function SupervisorPartnershipRequestCard({ 
  ...
  const displayName = isIncoming ? request.requesterName : request.targetSupervisorName;
  const displayEmail = isIncoming ? request.requesterEmail : request.targetSupervisorEmail;
    ...
    <div className={`${cardHover} ${borderLeftAccentBlue}`}>
      ...
      <div className={`${infoBoxBlue} mb-4`}>
        <p className={`text-sm ${textInfoDark}`}>
          {isIncoming 
            ? `${request.requesterName} wants to partner with you for co-supervising a specific project.`
            : `Waiting for ${request.targetSupervisorName} to respond to your partnership request for this project.`
          }
        </p>
        {request.projectId && (
          <p className={`text-xs ${textMuted} mt-1`}>
            Project ID: {request.projectId.substring(0, 8)}...
          </p>
        )}
      </div>
