...
export function generateProjectData(overrides?: Partial<Project>): Omit<Project, 'id'> {
  ...
  return {
    ...
    supervisorName: overrides?.supervisorName || 'Dr. Test Supervisor',
    coSupervisorId: overrides?.coSupervisorId ?? null,
    coSupervisorName: overrides?.coSupervisorName ?? null,
    ...
    createdAt: now,
    updatedAt: now,
    approvedAt: overrides?.approvedAt ?? (overrides?.status === 'approved' || overrides?.status === 'in_progress' || overrides?.status === 'completed' ? now : null),
    completedAt: overrides?.completedAt ?? (overrides?.status === 'completed' ? now : null),
    ...overrides,
  };
}
